...
async function loadEnrolledCourses() {
    ...
    try {
        ...
        if (data.success && data.courses && data.courses.length > 0) {
            ...
            container.innerHTML = '<p>You are not enrolled in any courses yet. Check "Available Courses" to join!</p>';
        }
    } catch (error) {
        console.error('ERROR in loadEnrolledCourses:', error);
        console.error('Error stack:', error.stack);
        container.innerHTML = `<p style="color:red;">Error: ${error.message}<br>Check console for details</p>`;
    }
}
...
async function loadAvailableCourses() {
    ...
    try {
        ...
        if (data.success && data.courses && data.courses.length > 0) {
            ...
            container.innerHTML = '<p>No available courses at the moment.</p>';
        }
    } catch (error) {
